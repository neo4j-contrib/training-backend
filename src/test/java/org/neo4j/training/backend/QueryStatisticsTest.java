...
public class QueryStatisticsTest {
    ...
    public void testNoUpdate() throws Exception {
        final CypherQueryExecutor.CypherResult result = cypherQueryExecutor.cypherQuery("start n=node("+refNode.getId()+") return n", null);
    ...
    public void testCreate() throws Exception {
        final CypherQueryExecutor.CypherResult result =
                cypherQueryExecutor.cypherQuery("start n=node("+refNode.getId()+") create n-[:FOO]->m", null);
        final int rowCount = result.getRowCount();
    ...
    public void testSet() throws Exception {
        final CypherQueryExecutor.CypherResult result =
                cypherQueryExecutor.cypherQuery("start n=node("+refNode.getId()+") set n.foo='bar'", null);
        final int rowCount = result.getRowCount();
        assertEquals(0, rowCount);
        final long time = result.getTime();
        assertTrue("time", time < 2000);
    ...
    public void testDelete() throws Exception {
        final CypherQueryExecutor.CypherResult result =
                cypherQueryExecutor.cypherQuery("start n=node("+refNode.getId()+") create n-[r:FOO]->m delete r,m", null);
        final int rowCount = result.getRowCount();
        assertEquals(0, rowCount);
        final long time = result.getTime();
        assertTrue("time", time < 2000);
